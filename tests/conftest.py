...
from khm_analyzer.cli.download_source import get_source_document_as_raw_bytes
...
def get_new_cacheable_content(edition, volume) -> str:
    uncachable_raw_content: bytes = get_source_document_as_raw_bytes(edition, volume)
    cacheable_content: str = encode_for_cache(uncacheable_raw_content)
    return cacheable_content

def get_cached_content_or_set_new_cache(edition: int, volume: int) -> bytes:
...
def khm_edition_volume(request, pytestconfig):
    ...
    cached = get_cached_content_or_set_new_cache(edition, volume)
    return cached
